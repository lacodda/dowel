...
## Marks with two colours

The line's palette of single hues ran out. A product founded after that owns
either one hue or a **pair**, drawn as one diagonal gradient across its tile,
its code and its banner rule.

The theme takes **the first colour of the pair and nothing else**. A gradient is
a property of the mark; an accent is a flat colour that fills a button, draws a
focus ring and sets a link. Carrying the pair into the vocabulary would make
every one of those a gradient, which is not what a pair is for — and a product
that wants its gradient anywhere else has the two values already, in its own
mark.
