...
<StatRow>
  ...
  <StatTile label="Silent" value={String(silent)} tone="warn" icon={<TriangleAlert className="size-4" />} />
</StatRow>
```
...
**A judgement brings a mark with it, and the type enforces that.** `warn` and
`bad` say the figure is itself a problem, and on a dashboard that claim rests
on colour alone — which is where it fails. The line's accents sit close to the
status hues: measured in OKLab, dowel's own accent is ΔE 6.1 from `--warn`,
kasl-server's gold is 3.7, hilvan's is 2.5 from `--bad`. An accented tile and a
warning one, three tiles apart in a row, are the same colour to a reader
glancing down it.

Neither side can move — the status hues are the line's shared language and the
accents come from the brand registry — so the rule the theme states in prose
becomes structure here: `tone="warn"` without an `icon` does not compile.

```tsx
// Will not build: a judgement drawn in colour alone.
<StatTile label="Silent" value="3" tone="warn" />
```

The first consumer had already worked this out by hand, wrapping its value in a
flex row with a warning triangle. That is the proof the requirement is real,
and the reason it belongs in the primitive rather than in a guideline nobody
reads twice. The mark is `aria-hidden`: it restates the tone, and the tone is
already in the label's words.
...
| `icon` | `ReactNode` | | A mark before the figure. **Required** with `warn` or `bad` |
