...
**The separators are the language's.** A thousand is `1,000` here, `1 000` there
...
locale is `useLocale()`'s answer — the nearest `LocaleProvider`, else the
page's `<html lang>`. It used to be the browser's language, which is how
kasl-server showed an English interface with Russian week headings to a reader
whose browser spoke Russian. Passing `'en-US'` to be safe is the opposite
mistake: a German reader shown American separators for the life of the
product. See [`no-implicit-locale`](/dowel/guides/linting/#no-implicit-locale).
