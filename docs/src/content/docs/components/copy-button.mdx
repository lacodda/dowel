---
title: CopyButton
description: The copy affordance in the corner of a block - which confirms only after the clipboard does, and says so out loud.
---

import Stand from '../../../components/Stand.astro'

```console
$ npx shadcn@latest add https://lacodda.github.io/dowel/r/copy-button.json
```

<Stand component="copy-button" caption="A button that appears on hover of the panel around it, and one whose clipboard refuses - which says so rather than drawing a tick." />
