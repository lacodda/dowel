...
description: One box that finds anything. dowel supplies the box, the filtering and the keyboard; the product supplies the commands.
---

import Stand from '../../../components/Stand.astro'

```console
$ npx shadcn@latest add https://lacodda.github.io/dowel/r/command-palette.json
```

<Stand component="command-palette" caption="Open and listing everything, narrowed by a query, and with nothing matching." />
...
Wrap the rows in `CommandPaletteGroup` with a `GroupLabel` when the box lists
more than one kind of thing, exactly as Combobox does.
