---
title: Scroll area
description: A box that scrolls with the line's overlay scrollbar - a thin bar drawn over the content, never a gutter that takes width - reachable and named for the keyboard.
---

import Stand from '../../../components/Stand.astro'

```console
$ npx shadcn@latest add https://lacodda.github.io/dowel/r/scroll-area.json
```

<Stand component="scroll-area" caption="A list taller than its box, the same list with edge fades, and a log that scrolls on both axes." />
