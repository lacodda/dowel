---
title: Select
description: A dropdown that is not the browser's, so it can be dressed like everything else.
---

import Stand from '../../../components/Stand.astro'

```console
$ npx shadcn@latest add https://lacodda.github.io/dowel/r/select.json
```

<Stand component="select" caption="Closed, open, grouped, and with more than one chosen." />
