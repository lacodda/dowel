---
title: Tabs
description: One place showing one of several views, or a window's title bar showing its open documents - with a close that does not nest one control inside another.
---

import Stand from '../../../components/Stand.astro'

```console
$ npx shadcn@latest add https://lacodda.github.io/dowel/r/tabs.json
```

<Stand component="tabs" caption="Line tabs over their panels, vertical tabs, and a window bar of open documents - one unsaved, each closable." />
