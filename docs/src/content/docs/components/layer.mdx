---
title: Layer
description: Which layer a popup opened inside an overlay lands on - so a select in a dialog, a menu in a drawer and a date picker in a popover open above what opened them.
---

import Stand from '../../../components/Stand.astro'

```console
$ npx shadcn@latest add https://lacodda.github.io/dowel/r/layer.json
```
...
<Stand component="layer" caption="A select in a dialog, a select in a popover, and a popover in a dialog with a select and a tooltip in it - each opening above what opened it." />
