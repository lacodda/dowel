---
title: Splitter
description: Resizable panes side by side or stacked, with a handle between each two that a pointer drags and a keyboard moves - sizes in percent, persisted by the product.
---

import Stand from '../../../components/Stand.astro'

```console
$ npx shadcn@latest add https://lacodda.github.io/dowel/r/splitter.json
```

<Stand component="splitter" caption="A collapsible file list beside an editor, two stacked panes, and three panes with a handle between each two." />
