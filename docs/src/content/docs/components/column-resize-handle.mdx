...
description: The strip on a header cell's edge that a column is dragged wider or narrower by, and the hook that keeps the widths - double-click resets.
---

import Stand from '../../../components/Stand.astro'

```console
$ npx shadcn@latest add https://lacodda.github.io/dowel/r/column-resize-handle.json
```

<Stand component="column-resize-handle" caption="A table whose headings end in a handle - drag one, double-click it to give the column its natural width back." />
