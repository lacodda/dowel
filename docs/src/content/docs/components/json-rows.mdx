---
title: json-rows
description: A JSON value flattened into rows, with the paths that name them - the arithmetic, with no React in it.
---

import Stand from '../../../components/Stand.astro'

```console
$ npx shadcn@latest add https://lacodda.github.io/dowel/r/json-rows.json
```

<Stand component="json-rows" caption="The rows a document flattens into, printed rather than drawn - with the keys that dot notation would break." />
...
```ts
interface JsonRow {
  path: string          // '$.items[3].name'
  key: string | null    // null only for the root
  index: boolean        // an array position rather than a name
  kind: JsonKind
  value?: string | number | boolean | null   // a leaf only
  depth: number
  size?: number         // a branch only: how many it holds
  parent?: string
}
```
