---
title: Wizard
description: A Stepper, the current step's content and Back / Next / Finish - with a check on every way forward, focus moved to each new step, and nothing lost on Back.
---

import Stand from '../../../components/Stand.astro'

```console
$ npx shadcn@latest add https://lacodda.github.io/dowel/r/wizard.json
```

<Stand component="wizard" caption="A workspace-setup wizard that checks each step, waits on a slow check, keeps what was typed on Back - above and beside its stepper." />
