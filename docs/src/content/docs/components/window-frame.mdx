---
title: WindowFrame
description: The window's own frame for a window with no system frame - the title bar with its documents, the three buttons, drag and double-click, and the edges you resize by.
---
...
<Stand component="window-frame" caption="A title bar holding its open documents, and the eight resize strips, drawn inside a frame - the stand runs in a browser, so they render and stay inert." />
